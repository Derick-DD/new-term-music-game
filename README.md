...
- 分享卡通过 QQ 音乐端内图片/链接分享面板发送；二维码指向正式活动地址并携带 `_hidehd=1&_miniplayer=1`
- 页面依次加载 QQ 音乐 `preact.js` 与 `music.js` 统计/端能力脚本；保留 polyfill，并将旧脚本的 `window.M` 映射为项目使用的 `window.Music`，避免与 music2 双重自动上报
